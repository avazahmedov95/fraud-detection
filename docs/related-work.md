...
| PR-AUC | 0.978 (0.970 ± 0.011 across seeds) | 0.380 |
...
|---|---|
| all 20 | 0.978 |
| amount + hour only | 0.691 |
| — PaySim, leak-free, 6 raw columns | 0.380 |

Feature poverty costs 0.978 → 0.691; the data itself accounts for 0.691 → 0.380.
...
project's 0.970. A PR-AUC in the high nineties is therefore not evidence of a
working system: it is the range a known-broken model reaches on public data. The
honest reading of 0.970 is not "better than the benchmark" but "the same class of
