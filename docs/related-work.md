...
| PR-AUC | 0.947 (0.967 ± 0.018 across seeds) | 0.380 |
...
|---|---|
| all 24 | 0.947 |
| amount + hour only | 0.581 |
| — PaySim, leak-free, 6 raw columns | 0.380 |

Feature poverty costs 0.947 → 0.581; the data itself accounts for 0.581 → 0.380.
...
project's 0.967. A PR-AUC in the high nineties is therefore not evidence of a
working system: it is the range a known-broken model reaches on public data. The
honest reading of 0.967 is not "better than the benchmark" but "the same class of
