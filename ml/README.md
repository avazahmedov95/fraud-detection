...
**Class weighting stops where it stops working.** `train.py` weights fraud by
negatives over positives above 0.5% fraud - validated at the baseline profile's
1.5% - and fits unweighted below it, where every weight measured did worse than
none (the realistic profile, IBM AML, PaySim; figures in the section after the
table). `CLASS_WEIGHTING=on` or `off` forces either. An unweighted model's
probabilities sit near the base rate, which is why the job's cutoffs are chosen
on validation rows and shipped as `thresholds.json` rather than fixed. The tests
are in `tests/`.
...
Numbers are regenerated by `train.py`; the authoritative copy is
`models/metrics.json`, not this table - and `tools/boundary_audit.py` fails if the
two disagree, so the table cannot quietly stop being true.
...
Three things changed in the recipe, each measured before it was adopted:
...
`stream-processor/capabilities.py` for the switches. Every figure in this section
...
above legitimate traffic* (it does - ROC-AUC 0.991 / PR-AUC 0.472 on the realistic
profile) but *are its
...
still could not **order** an alert queue, because the alerts piled at the top of
the scale. On the realistic profile 11.9% round to 1.000 and 160 alerts carry 115
distinct scores - the queue can be ordered again. AUC is blind to this by construction -
