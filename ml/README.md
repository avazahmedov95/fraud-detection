...
**Class weighting has a floor.** `train.py` weights fraud by negatives over
positives, validated at this project's 1.5% (a weight near 65). On PaySim (0.13%)
and IBM AML (0.10%) the same weighting collapsed the model, and real card traffic
runs near 0.17% (`validation/README.md` 1, 2, 4). Below 0.5% fraud, training
therefore stops instead of saving a collapsed model, unless told how to weight:
`CLASS_WEIGHTING=off` fits unweighted - the recipe that held on both datasets -
after which the decision thresholds must be set again from a validation slice,
because the 0.50 cutoff flags too little; `CLASS_WEIGHTING=on` weights anyway.
0.5% is a guard between the two measurements, not a measured boundary. The tests
are in `tests/`, the first this package has had.
