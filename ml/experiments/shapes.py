...
Both passed; IBM AML's cutoff menu then found the gain confined to the top of the
list, its people-like transfers showed the same, and the build was stopped
(ml/README.md). The owner keeps a strict cutoff, so a third question was added
afterwards - narrower than the first two and chosen after seeing IBM AML, which is
why it is asked of data not looked at yet:
  3. PaySim, the published split's first 24 days - fit on their earliest 80% of
     rows, validate on the rest - unweighted, five seeds. On the VALIDATION rows
     (a) the paired per-seed PR-AUC difference is not below zero on the mean, and
     (b) the averaged committee finds at least as much fraud in the top 0.1% of
     transfers with the shapes as without. Both hold: the shapes are built into
     the stream for a strict cutoff, with IBM AML's many-alert caveat recorded.
     Either fails: they are not built. The last seven days are reported and
     decide nothing.
    ...
    python experiments/shapes.py --paysim ../validation/PS_20174392719_1491204439457_log.csv
"""
import argparse
import os
import sys
...
def realistic(cache, seeds):
    ...
    print(f"realistic profile: {n:,} rows; fit {fit:,}, validation {cut - fit:,} "
          ...
          f"weight {spw:.0f}")
    _describe(X[:, k:], y)
    dv, committee_ok, _ = _compare(X, y, list(range(k)), fit, cut, seeds, spw, types)
...
def ibm(path, cache, seeds):
    ...
    _describe(S, y)
    dv, _, _ = _compare(X, y, list(range(base.shape[1])), a, b, seeds, 1.0)
    print()
    print("verdict, as fixed in the module docstring before the run:")
...
def paysim(path, seeds):
    ...
    # PaySim's own profile, as paysim_adapter.py sets it: what PaySim lacks is off.
    ...
    keep, _ = P.RP.available_features()
    Xb, y, _ = P.RP.extract_features(P.to_events(df, P.scale_factor(df.amount)),
                                     total=len(df))
    base, y = Xb[:, keep], np.asarray(y).astype("int8")
    ids, _ = pd.factorize(np.concatenate([df.nameOrig.values, df.nameDest.values]))
    S = shape_features(df.step.values.astype("int64") * 3600, ids[:len(df)], ids[len(df):])
    X = np.hstack([base, S])
    cut = int((df.step.values <= P.BASELINE["cut_step"]).sum())
    fit = int(cut * 0.8)
    print(f"PaySim: {len(y):,} rows; fit {fit:,}, validation {cut - fit:,} "
          f"({int(y[fit:cut].sum()):,} fraud), last seven days {len(y) - cut:,} "
          f"({int(y[cut:].sum()):,} fraud)")
    _describe(S, y)
    dv, _, res = _compare(X, y, list(range(base.shape[1])), fit, cut, seeds, 1.0)
    yva = y[fit:cut]
    top = {k: _top_share(yva, np.mean(va, axis=0), 0.001) for k, (va, _) in res.items()}
    print(f"  top 0.1% of the validation transfers: without finds {top['without']:.1%}, "
          f"with finds {top['with']:.1%}")
    passed = dv.mean() >= 0 and top["with"] >= top["without"]
    print()
    print("verdict, as fixed in the module docstring before the run:")
    print("  on PaySim the shapes " + ("PASS - build them into the stream for a strict cutoff"
                                       if passed else "do NOT pass - not built"))
...
def main():
    ...
    ap.add_argument("--paysim", help="PaySim's CSV: ask the third question instead")
    args = ap.parse_args()
    if args.paysim:
        return paysim(args.paysim, args.seeds)
    if args.ibm:
