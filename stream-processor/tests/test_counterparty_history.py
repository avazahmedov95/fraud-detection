...
def test_the_window_is_an_hour(on):
    """Half an hour is transit, two hours is not: the boundary of attempt three."""
    now = 1_000_000.0
    inside = _res(_ev(amount=1_000_000), now=now, paid_at=now - 1_800,
                  inflow=1_000_000)
    outside = _res(_ev(amount=1_000_000), now=now, paid_at=now - 7_200,
                   inflow=1_000_000)
    assert "PASS_THROUGH" in inside["rule_hits"]
    assert "PASS_THROUGH" not in outside["rule_hits"]
